...
//        HAL_SPI_Transmit_DMA(&hspi2,&buf[0],1);//页地址（从0xB0到0xB7）
//        HAL_SPI_Transmit_DMA(&hspi2,&buf[1],1);//起始列地址的高4位
//        HAL_SPI_Transmit_DMA(&hspi2,&buf[2],1);//起始列地址的低4位
        for (j = 0; j < 8; j++) { //整页内容填充
            buf[3] = ASCII_8x16[(w * 16) + c - 512];
            OLED_WR_Byte(buf[3], OLED_CMD);
//            HAL_SPI_Transmit_DMA(&hspi2,&buf[3],1);//为了和ASII表对应要减512
            c++;
        }
        x++; //页地址加1
    }
...
void OLED_printf(uint8_t row, char *fmt, ...)
{
    ...
    vsnprintf(buff, 17, fmt, art_ptr);   //数据转换
